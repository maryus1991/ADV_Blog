...
class TestBlogViewAndUrl(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(email="post@post.com", password="1234")
        user = self.user
        ...
        client.login(email="post@post.com", password="1234")
        self.client = client
    ...
    def test_post_detail_view_with_get_method_and_url(self):
        ...
        response = client.post(
            url,
            data={"full_name": "mez", "email": "test@test.com", "comment": "test test"},
        )
        self.assertEqual(response.status_code, 302)

        # check the comment in database
        comment = PostsComment.objects.filter(
            post=post, full_name="mez", email="test@test.com"
        )
    ...
    def test_edit_and_delete_comments_and_test_there_urls(self):
        ...
        response = client.get(edit_comment_url)
        self.assertEqual(response.status_code, 200)
        ...
        response = client.post(delete_comment_url, data={"comment_id": comment.id})

        self.assertTrue(response.status_code == 302)

        # check again if the object still exist in db
        self.assertFalse(PostsComment.objects.filter(id=comment.id).exists())
    ...
    def test_create_and_delete_posts_and_test_urls_and_views(self):
        ...
        response = client.post(delete_post_url)
