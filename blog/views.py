...
class PostsDetailViews(DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse("PostsDetailViews", kwargs={"pk": self.get_object().id})
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["most_view_post"] = (
            Post.objects.order_by("view")
            .annotate(most_view_post_count=Count("view"))
            .all()[:5]
        )

        # adding the parent comments
        context["comments"] = (
            PostsComment.objects.filter(parent=None, post=self.get_object())
        ...
        if request.method == "POST":
            ...
            if comment_id is not None:
                ...
                if (
                    ...
                    comment.save()
                    messages.success(request, "نظر مورد نظر با موقیت حذف شد")

                else:
                    messages.error(request, "شما اجازه حدف این نظر را ندارید")
...
class EditPostComment(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if email == comment.email:
            ...
            return render(
                request,
                "blog/editComment.html",
                {"post": comment.post, "comment": comment, "comment_form": form},
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if comment is not None:
                ...
                messages.error(request, "کامنت پیدا نشد")
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        *set the dispatch func for check the user is active and verified his account
        ...
        if (
            ...
            messages.error(request, "لطفا وارد حساب کاربری خود شوید و ان را فعال کنید")
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        *set the dispatch func for check the user is active and verified his account
        ...
        if (
            ...
            if Post.objects.get(id=kwargs.get("pk")).author.id == request.user.id:

                return super().dispatch(request, *args, **kwargs)
            else:
                messages.error(request, "شما نویسنده این مقاله نیستید")

            return redirect(reverse("Dashboard"))

        else:
            # set and redirect the user to dashboard if not active and not verify
            messages.error(request, "لطفا اکانت خود را فعال کنید")
...
class DeletePost(LoginRequiredMixin, View):
    """
    for deactivate the post from showing for people
    """
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        *set the dispatch func for check the user is active and verified his account
        ...
        if (
            ...
            # check the user that the should be the post author or admin
            if (
                Post.objects.get(id=kwargs.get("pk")).author.id == request.user.id
            ) or request.user.is_superuser:

                return super().dispatch(request, *args, **kwargs)

            else:
                # set error for user not author
                messages.error(request, "شما نویسنده این مقاله نیستید")
    ...
    def post(self, request, *args, **kwargs):
        # set the post method for deactivating the post
        ...
        # updating the post object and saving
        post.delete()

        # set success full message
        messages.success(request, "پست شما با موفقیت حذف شد")

        return redirect(reverse("PostsListsViews"))
...
def header_render_partial(request):
    ...
    return render(request, "layouts/Header/Header.html", {"SiteSetting": SiteSetting})


def footer_render_partial(request):
    # partial render for footer
    return render(request, "layouts/Footer/Footer.html", {"SiteSetting": SiteSetting})
