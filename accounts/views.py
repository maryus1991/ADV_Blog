...
class Dashboard(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # updating user email
        context["UpdateEmailForm"] = UpdateEmailForm(
...
class Registrations(View):
    ...
    def post(self, request, *arg, **kwargs):
        ...
        if not request.user.is_authenticated:
            ...
            if form.is_valid():
                ...
                if not user:
                    ...
                    context = {
                        ...
                        + reverse(
                            "ConformAccount", kwargs={"token": user.verified_code}
                        )
                    }
                    ...
                    messages.warning(
                        request, "کاربری با این مشخصات قبلا ثبت نام کرد است"
                    )
                    return redirect(reverse("Authorizations"))
            ...
            messages.warning(request, "شما وارد سایت هستید")
            return redirect(reverse("Dashboard"))
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not request.user.is_authenticated:
            ...
            if form.is_valid():
                ...
                messages.error(request, "لطفا اطلاعات را به صورت دقیق وارد نمایید")
                return redirect(reverse("Authorizations"))
...
class Logout(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not request.user.is_authenticated:
            ...
            messages.warning(request, "شما باید وارد سایت شوید")
            return redirect(reverse("Authorizations"))
...
class ConformAccount(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        # adding message
        messages.success(self.request, "حساب شما با موفقیت فعال شد")
...
class ForgotPassword(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        """
        if request.user.is_authenticated:
            messages.warning(request, "لطفا اول از حساب کاربری خود خارج شوید")
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "accounts/ForgotPassword_SendPassword.html",
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                context = {
                    "url": str(request.get_host())
                    + reverse(
                        "ForgotPassword_token", kwargs={"token": user.verified_code}
                    )
                }

                # create email object
                email_message = EmailMessage(
                    "Email/ForgotPassword.tpl",
                    context,
                    settings.EMAIL_HOST_USER,
                    [user.email],
                )
                ...
                messages.success(
                    request, f"ایمیل با موفقیت برای ایمیل ( {email} ) شما ارسال شد"
                )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                messages.error(request, "کابر یافت نشد")
                return redirect(reverse("Authorizations"))
...
class ChangePassword(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        if current_password_form.is_valid():
            ...
            # get the password
            current_password = current_password_form.cleaned_data.get(
                "current_password"
            )
            ...
            if user.check_password(current_password):
                ...
                if password.is_valid():
                    ...
                    if current_password == new_password:
                        messages.error(request, "رمز های عبور های شما یکسان هستند")
                    ...
                    messages.error(request, password.errors)
                    return reverse("Dashboard")

            else:
                # setting message for wrong pass
                messages.error(request, "رمز عبور شما اشتباه است")
                return reverse("Dashboard")

        else:
            # invalid error set
            messages.error(request, current_password_form.errors)
            return reverse("Dashboard")
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        if email_form.is_valid():
            ...
            if email == request.user.email:
                messages.error(request, "ایمل جدید با ایمیل قبلیتان یکسان هستن")
                return reverse("Dashboard")
            ...
            if exist_user:
                ...
                messages.error(request, "این ایمیل وجود دارد")
                return reverse("Dashboard")
            ...
            # set message and redirect th user
            messages.success(
                request, f"لینک فعال سازی برای این ایمیل ( {user.email} ) ارسال شد"
            )
...
class ResentEmail(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        """
        return render(
            request, "accounts/ResentEmail.html", {"UpdateEmailForm": UpdateEmailForm}
        )
