...
class TestAccountAuthenticatedApi:
    ...
    def test_jwt_verify_url_and_jwt_refresh_url_with_valid_and_invalid_user_information(
        self, client
    ):
        ...
        refresh_url_response = client[1].post(
            refresh_url, {"refresh": client[2].get("refresh")}
        )
        verify_url_response = client[1].post(
            verify_url, {"token": client[2].get("access")}
        )
        ...
        verify_url_response = client[1].post(
            verify_url, {"token": get_random_string(255)}
        )
        refresh_url_response = client[1].post(
            refresh_url, {"refresh": get_random_string(255)}
        )

        assert verify_url_response.status_code == 401
        assert refresh_url_response.status_code == 401

    def test_login_and_logout_token_authentication_and_get_user_information_urls_with_user_information(
        self, client
    ):
        ...
        AuthenticatedUserInformation_API_url = reverse(
            "AuthenticatedUserInformation_API"
        )

        # get the token from token login api
        CustomAuthenticationsToken_API_response = client[1].post(
            CustomAuthenticationsToken_API_url, user_options
        )
    ...
    def test_user_change_password_and_user_change_email_and_user_update_profile_and_get_user_information_urls_with_valid_data(
        self, client
    ):

        # get the urls

        UserInformationShowing_API_url = reverse(
            "UserInformationShowing_API", kwargs={"pk": client[3].pk}
        )
        UserChangePassWord_API_url = reverse(
            "UserChangePassWord_API", kwargs={"pk": client[3].pk}
        )
        UserChangeEmail_API_url = reverse(
            "UserChangeEmail_API", kwargs={"pk": client[3].pk}
        )
        UserChangeProfile_API_url = reverse(
            "UserChangeProfile_API", kwargs={"pk": client[3].pk}
        )
        ...
        # send request
        response = client[0].put(
            UserChangeEmail_API_url, {"email": "admin@amdin.admin"}
        )
        ...
        assert (
            client[0]
            .put(UserChangeEmail_API_url, {"email": user_options.get("email")})
            .status_code
            == 200
        )
        ...
        response = client[0].put(
            UserChangeProfile_API_url, {"first_name": "mez", "last_name": "mez"}
        )
    ...
    def test_forgot_password_and_rent_email_api_with_anonymous_request_and_valid_data(
        self, client
    ):
        ...
        # send request
        response = client[1].post(
            ForgotPassword_API_url, {"email": user_options.get("email")}
        )
        ...
        # send request
        response = client[1].post(
            resent_email_api_url, {"email": user_options.get("email")}
        )
