...
class TestAccountsUrls(TestCase):
    ...
    def setUp(self):
        ...
        user = User.objects.create_user(
            email="admin@admin.domain", password="@Aa1234567890"
        )
    ...
    def test_dashboard_url_with_login_and_anonymous_requests(self):
        ...
        client = Client()
        client.login(email="admin@admin.domain", password="@Aa1234567890")
    ...
    def test_registrations_urls_with_post_method_and_valid_and_invalid_data(self):
        ...
        )  # check the post request with sended data
        self.assertEqual(
            client.get(url).status_code, 302
        )  # check the get method with request
        self.assertEqual(
            client.post(url).status_code, 302
        )  # check the post method with invalid data
    ...
    def test_login_urls_with_post_and_get_method_with_valid_and_invalid_data(self):
        ...
        # set the data
        response = client.post(
            url, data={"email": "admin@admin.domain", "password": "@Aa1234567890"}
        )
    ...
    def test_forgot_password_urls_and_check_the_user_password(self):
        ...
        login_client.login(email="admin@admin.domain", password="@Aa1234567890")
        ...
        response = anonymous_client.post(
            forqot_password_url, data={"email": "maryus19915123@gmail.com"}
        )
        self.assertEqual(response.status_code, 302)
        ...
        self.assertEqual(
            anonymous_client.post(
                reverse(
                    "ForgotPassword_token", kwargs={"token": get_random_string(255)}
                )
            ).status_code,
            302,
        )
    ...
    def test_update_password_and_update_profile_urls_with_user_data(self):
        ...
        update_profile_url_response = login_client.post(
            update_profile_url,
            data={"first_name": "kazem", "last_name": "yesman", "avatar": ""},
        )

        change_password_url_response = login_client.post(
            change_password_url,
            data={
    ...
    def test_change_email_and_resent_email_urls_with_user_data(self):
        ...
        response = login_client.post(
            ChangeEmail_url, data={"email": "admin@admin.domain"}
        )
